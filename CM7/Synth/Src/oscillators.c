...
/*------------------------------------------------------------------------*/
Oscillator_t op1 _DTCMRAM_;
Oscillator_t op2 _DTCMRAM_;
...
/*---------------------------------------------------------------*/
float_t _ITCMRAM_ Drifters_sampleCompute(void) // 3 drifting sawtooths with "fixed" main pitch in centralFreq variable
{
	float y;
...
/*----------------------------------------------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute(Oscillator_t *op) // accurate sine
{
	float_t z;
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ Osc_WT_SINE_SampleCompute(Oscillator_t *op) // basic wave table sine
{
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute1(Oscillator_t *op) // basic sawtooth^2
{
	while (op->phase < 0) // keep phase in [0, 2pi]
		op->phase += _2PI;
	while (op->phase >= _2PI)
	...
	if (op->phase < _PI)
		...
		op->out = -.5f * (op->amp * (op->phase / _PI - 2) * (op->phase / _PI - 2));

	op->phase += _2PI * Ts * op->freq; // increment phase
	return op->out;
}
/*-------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute2(Oscillator_t *op) // basic sawtooth
{
	while (op->phase < 0) // keep phase in [0, 2pi]
		op->phase += _2PI;
	while (op->phase >= _2PI)
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute3(Oscillator_t *op) // sin(phi)^5
{
	float_t z;
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute4(Oscillator_t *op) // Complex waveform : +/- |sin(phi)|^alpha(freq), tends to a sine at high freqs
{
	float_t z, x, alpha;
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ OpSampleCompute5(Oscillator_t *op) // Naive Triangle
{
	while (op->phase < 0) // keep phase in [0, 2pi]
		op->phase += _2PI;
	while (op->phase >= _2PI)
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ MorphingSaw_SampleCompute(Oscillator_t *op) // Morphing sawtooth, tends to a triangle at high freqs
{
	while (op->phase < 0) // keep phase in [0, 2pi]
		op->phase += _2PI;
	while (op->phase >= _2PI)
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ BasicSquare_SampleCompute(Oscillator_t *op) // naive square
{
...
/*-------------------------------------------------------*/
float_t _ITCMRAM_ AdditiveGen_SampleCompute(Oscillator_t *op) // additive sine generator
{
	uint8_t k = 1;
	float_t y = 0;
...
float _ITCMRAM_ waveCompute(uint8_t sound, float frq)
{
	...
	{	// major chord : 1 3maj 5
		...
		y = 0.33f
				* (MorphingSaw_SampleCompute(&op1) + MorphingSaw_SampleCompute(&op2) + MorphingSaw_SampleCompute(&op3));
	}
		break;
