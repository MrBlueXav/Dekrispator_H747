...
static Lfo_t lfoL _DTCMRAM_, lfoR _DTCMRAM_; // 2 independent LFOs
static monochorus_t delR _DTCMRAM_, delL _DTCMRAM_; // 2 fractional delay lines
static float rateCoeff _DTCMRAM_;

/* Private function prototypes -----------------------------------------------*/

float _ITCMRAM_ mono_chorus_compute(monochorus_t *del, Lfo_t *lfo, float xin);

/********************************************* CHORUS interface ************************************************************/

/*--------------------This is the main stereo chorus function : ----------------------------*/

void _ITCMRAM_ stereoChorus_compute(float *left_out, float *right_out, float in)
{
	*left_out = mono_chorus_compute(&delL, &lfoL, in);
	*right_out = mono_chorus_compute(&delR, &lfoR, in);
}

void Chorus_params_set(const ChorusParams_t *params)
{

	ChorusDelay_init(&delL, params->Ldelay, params->Lfdb, params->Lforward, params->Lmix);
	ChorusDelay_init(&delR, params->Rdelay, params->Rfdb, params->Rforward, params->Rmix);
	lfoL.amp = params->lfoL_amp;
	lfoR.amp = params->lfoR_amp;
	lfoL.freq = params->lfoL_freq;
	lfoR.freq = params->lfoR_freq;
	lfoL.phase = _PI / 2; // initial phases for quadrature
	lfoR.phase = 0;
	rateCoeff = params->rateCoeff;
	delL.in_idx = params->Lidx;
	delR.in_idx = params->Ridx;
	delL.mode = delR.mode = params->mode;
}

void Chorus_params_save(ChorusParams_t *params)
{
	params->lfoL_amp = lfoL.amp;
	params->lfoR_amp = lfoR.amp;
	params->lfoL_freq = lfoL.freq;
	params->lfoR_freq = lfoR.freq;
	params->rateCoeff = rateCoeff;
	params->Ldelay = delL.baseDelay;
	params->Rdelay = delR.baseDelay;
	params->Lfdb = delL.fb;
	params->Rfdb = delR.fb;
	params->Lmix = delL.mix;
	params->Rmix = delR.mix;
	params->Lforward = delL.fw;
	params->Rforward = delR.fw;
	params->Lidx = delL.in_idx;
	params->Ridx = delR.in_idx;
	params->mode = delL.mode;
}
...
/*************************************** Private computing functions ******************************************/

/*-------------------------------------------------------------------------------------------*/
float _ITCMRAM_ Lfo_SampleCompute(Lfo_t *op) // ! returns a positive value between 0 and op.amp !
{
	float z;
...
/*-------------------------------------------------------------------------------------------*/
float _ITCMRAM_ delay_read(monochorus_t *del, float delay) // "delay" is a floating point number of samples
{
	...
	return (f - 2) * (f - 3) * (-0.16666666666f * (f - 1) * y_n + 0.5f * f * y_n_1)
			+ f * (f - 1) * (-0.5f * (f - 3) * y_n_2 + 0.166666666666f * (f - 2) * y_n_3);
