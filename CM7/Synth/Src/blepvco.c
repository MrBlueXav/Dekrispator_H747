...
//----------------------------------------------------------------------------------------------------------
void _ITCMRAM_ place_slope_dd(float *buffer, int index, float phase, float w, float slope_delta)
{
	float r;
	int i;
...
float _ITCMRAM_ VCO_bleprect_SampleCompute(VCO_bleprect_t *vco)
{
	int j, k;
	float freq, syncin;
	...
	{ /* normal operation, signal currently high */
	...
	{ /* normal operation, signal currently low */
...
float _ITCMRAM_ VCO_blepsaw_SampleCompute(VCO_blepsaw_t *vco)
{
	int j;
	float freq, syncin;
	float a, p, t, w, dw, z;
...
//----------------------------------------------------------------------------------------------------------
float _ITCMRAM_ VCO_bleptri_SampleCompute(VCO_bleptri_t *vco)
{
	int j, k;
	float freq, syncin;
	...
	{ /* normal operation, slope currently up */
	...
	{ /* normal operation, slope currently down */
...
////----------------------------------------------------------------------------------------------------------
float _ITCMRAM_ Blepsaw_SampleCompute(BlepOscillator_t *vco)
{
	int j;
	float freq, syncin;
	float a, p, t, w, dw, z;
...
//----------------------------------------------------------------------------------------------------------
float _ITCMRAM_ Bleprect_SampleCompute(BlepOscillator_t *vco)
{
	int j, k;
	float freq, syncin;
	float a, b, db, p, t, w, dw, x, z;
...
//----------------------------------------------------------------------------------------------------------
float _ITCMRAM_ Bleptri_SampleCompute(BlepOscillator_t *vco)
{
	int j, k;
	float freq, syncin;
	float a, b, b1, db, p, t, w, dw, x, z;
...
void BlepOsc_init(BlepOscillator_t *osc, BlepType_t type)
{
	...
	osc->_z = 0.0f;
	osc->_j = 0;
	memset(osc->_f, 0, (FILLEN + STEP_DD_PULSE_LENGTH) * sizeof(float));
}

//----------------------------------------------------------------------------------------------------------
void BlepOsc_params_set(const BlepOscillatorParams_t *params, BlepOscillator_t *osc)
{
...
//----------------------------------------------------------------------------------------------------------
void BlepOsc_params_save(const BlepOscillator_t *osc, BlepOscillatorParams_t *params)
{
	params->type = osc->type;
	params->amp = osc->amp;
	params->last_amp = osc->last_amp;
	params->waveform = osc->waveform;
	params->freq = osc->freq;
}

//----------------------------------------------------------------------------------------------------------
float BlepOsc_sampleCompute(BlepOscillator_t *vco)
{
	switch (vco->type)
	{

	case BLEP_SAW:
		return Blepsaw_SampleCompute(vco);
		break;

	case BLEP_RECT:
		return Bleprect_SampleCompute(vco);
		break;

	case BLEP_TRI:
		return Bleptri_SampleCompute(vco);
		break;
